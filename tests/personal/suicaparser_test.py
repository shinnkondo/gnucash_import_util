...
class TestSuicaParser(TestCase):

    def setUp(self):
        self.parser = SuicaParser()

    def test_is_applicable(self):
        self.assertTrue(self.parser.is_applicable("カードID,00000000000000"))

    def test_extract_fields_negative(self):
        row = ["2018/07/30", "自動販売機", "物品購入", "","","","","268","4881",""]
        self.assertEqual(self.parser.extract_fields(row),
    ...
    def test_choose_given_account_card(self):
        info =  PartialTransactionInfo(
                ...
                "Charge",
                3000)
        self.assertEqual(self.parser.choose_giving_account(info), MyAccount.CARD)
    ...
    def test_choose_given_account_trans(self):
        info =  PartialTransactionInfo(
            ...
            "自動改札機",
            -200)
        self.assertEqual(self.parser.choose_giving_account(info), MyAccount.TRANSPORTATION)
    ...
    def test_choose_given_account_unknown(self):
        info =  PartialTransactionInfo(
            ...
            "Dining",
            -2000)
        self.assertEqual(self.parser.choose_giving_account(info), MyAccount.UNKNOWN)
    ...
    def test_choose_given_account_meals(self):
        ...
        "Dining",
        -500)
        self.assertEqual(self.parser.choose_giving_account(info), MyAccount.MEALS)
    ...
    def test_choose_given_account_snack(self):
        ...
        "Dining",
        -100)
        self.assertEqual(self.parser.choose_giving_account(info), MyAccount.SNACK)
